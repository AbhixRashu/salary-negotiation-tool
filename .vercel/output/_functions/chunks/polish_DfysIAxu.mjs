...
//#region src/pages/api/polish.ts
var polish_exports = /* @__PURE__ */ __exportAll({ POST: () => POST });
var SYSTEM_PROMPT = `You are a world-class salary negotiation strategist. You help professionals refine their negotiation emails to be more persuasive, concise, and effective. You combine Harvard Negotiation Project methodology, FBI behavioral tactics, and real HR insider knowledge.`;
var POLISH_PROMPTS = {
...
var POST = async ({ request }) => {
	try {
		const { email, mode, context } = await request.json();
		if (!email || !mode || !POLISH_PROMPTS[mode]) return new Response(JSON.stringify({ error: "Invalid request. Provide email and valid mode." }), {
			status: 400,
...
Return ONLY the polished email. No explanations, no meta-commentary, no subject line. Start directly with "Hi [Name]," or "Hello [Name]," and end with a professional sign-off.`;
		...
		console.log(`[Polish Request] ${(/* @__PURE__ */ new Date()).toISOString()} | mode: ${mode} | keys: ${geminiKeys.length}`);
		...
		return new Response(JSON.stringify({
			text: generateFallbackPolish(email, mode),
			isFallback: true
		}), {
			status: 200,
			headers: { "Content-Type": "application/json" }
...
async function callGemini(prompt, apiKey) {
	try {
		...
		if (err.name === "AbortError") console.error("Gemini Polish call timed out after 8.5s");
...
async function callClaude(prompt, apiKey) {
	try {
		...
		if (err.name === "AbortError") console.error("Claude Polish call timed out after 8.5s");
		else console.error("Claude call failed:", err);
		return null;
	}
}
function generateFallbackPolish(email, mode) {
	const paragraphs = email.split(/\n\s*\n/).filter((p) => p.trim());
	const last = paragraphs.length - 1;
	const transforms = {
		"more-assertive": (ps) => [
			ps[0],
			...ps.slice(1, last + 1).map((p) => p.replace(/\bI (?:think|believe|would (?:like|love) to|hope to)\b/gi, "I").replace(/\bI (?:am|'m) (?:confident|sure) that\b/gi, "My baseline is that").replace(/\bwould (?:love|like) to\b/gi, "will").replace(/\bcould we\b/gi, "we must").replace(/\bI was hoping\b/gi, "I require").replace(/\bI'd love to\b/gi, "I need to").replace(/\bI think\b/gi, "I know").replace(/\b(?:perhaps|maybe)\b/gi, "")),
		...
		"warmer-tone": (ps) => [
			...
			...ps.slice(1, last + 1).map((p) => p.replace(/\bI (?:require|need)\b/gi, "I'd love to").replace(/\bmy baseline is\b/gi, "I was hoping we could").replace(/\bI require\b/gi, "would it be possible to").replace(/\bI know\b/gi, "I feel").replace(/\bI will\b/gi, "I'd love to")),
		...
		"add-market-data": (ps) => ps.map((p, i) => i === last ? p + "\n\nBased on current market data for this role in this location, the 75th percentile aligns with my target — this isn't a request, it's simply market reality. I hope you can see this as a data-driven request rather than a demand." : p),
		"fix-grammar": (ps) => ps.map((p) => p.replace(/\s+/g, " ").replace(/\.\s*\./g, ".").replace(/,\s*,/g, ",").replace(/\bI\s+I\b/gi, "I").replace(/\bi\b/gi, "I").replace(/\b(?:dont|cant|wont|isnt|wasnt|didnt|doesnt|havent|hasnt|couldnt|wouldnt|shouldnt)\b/gi, (m) => m.toLowerCase().replace("nt", "n't")).trim()),
		"expand-details": (ps) => ps.map((p, i) => i === Math.min(1, last) ? p + " This result came from leading a cross-functional team of 8 and delivering 3 weeks ahead of schedule — the kind of execution I'll bring to this role from day one." : i === last ? p + "\n\nBeyond the numbers, I bring deep domain expertise, strong cross-functional leadership, and a track record of turning around challenging projects. I'm excited to bring all of this to the team at " + (ps[0]?.match(/at\s+(\w+)/i)?.[1] || "your company") + "." : p)
	};
	return (transforms[mode] || transforms["fix-grammar"])(paragraphs).join("\n\n");
}
//#endregion
//#region \0virtual:astro:page:src/pages/api/polish@_@ts
var page = () => polish_exports;
//#endregion
export { page };
